...
export default function PeriodComparisonChart({
  ...
  return (
    <div className="surface-card glass-card rounded-2xl p-4 md:p-6">
      {/* Header */}
...
function DeltaBadge({ label, delta }: { label: string; delta: number }) {
  ...
  return (
    <div className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium ${positive ? "border-success/30 bg-success/10 text-success" : "border-danger/30 bg-danger/10 text-danger"}`}>
      <span>{positive ? "▲" : "▼"}</span>
