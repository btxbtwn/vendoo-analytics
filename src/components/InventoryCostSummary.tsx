...
export default function InventoryCostSummary({ data, compact = false }: InventoryCostSummaryProps) {
  const cards = [
    ...
    { label: "Active Inventory Cost", value: fmtCurrency(data.activeInventoryCost), sub: `${data.activeCount} active`, highlight: true },
    ...
    { label: "Sell-Through", value: fmtPct(data.totalCount > 0 ? (data.soldCount / data.totalCount) * 100 : 0), sub: `${data.soldCount} of ${data.totalCount}` },
  ];
      ...
      <div className="grid grid-cols-2 gap-3">
        {cards.map((card) =>
          card.highlight ? (
            <div
              key={card.label}
              className="card rounded-2xl border-2 border-accent bg-accent/10 p-4"
            >
              <dt className="text-xs text-accent font-medium">{card.label}</dt>
              <dd className="mt-1 font-heading text-2xl font-bold tabular-nums tracking-tight text-accent">
                {card.value}
              </dd>
              <p className="mt-0.5 text-xs text-muted-foreground">{card.sub}</p>
            </div>
          ) : (
            <div key={card.label} className="card rounded-2xl border border-border bg-card p-4">
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
      {cards.map((card) =>
        card.highlight ? (
          <div
            key={card.label}
            className="card rounded-2xl border-2 border-accent bg-accent/10 p-5 md:p-6"
          >
            <dt className="text-xs text-accent font-medium">{card.label}</dt>
            <dd className="mt-2 font-heading text-2xl font-bold tabular-nums tracking-tight text-accent md:text-3xl">
              {card.value}
            </dd>
          ...
          <div
            key={card.label}
            className="card rounded-2xl border border-border bg-card p-5 md:p-6"
          >
            <dt className="text-xs text-muted-foreground">{card.label}</dt>
